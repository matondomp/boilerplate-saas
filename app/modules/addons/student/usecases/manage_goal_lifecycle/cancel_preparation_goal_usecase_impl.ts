...
export interface CancelPreparationGoalInput {
  userId: string
  goalId: string
}

export class CancelPreparationGoalUseCaseImpl {
  constructor(
  ...
  async perform(input: CancelPreparationGoalInput): Promise<Either<any, boolean>> {
    ...
    const transitionResult = goal.cancel()

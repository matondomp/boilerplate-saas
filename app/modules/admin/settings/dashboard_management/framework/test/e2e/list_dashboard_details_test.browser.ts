...
test.group('Dashboard details', (group) => {
  group.each.setup(async () => {
    await testUtils.db().truncate()
  })

  test('Should list dashboard attached items', async ({ visit }) => {
    ...
    await page.assertUrlContains(dashboardDetailsUrl)

    // const element = await page.getByText(dashboardItemModel.name)
    // await page.assertVisible(element)
  })
})
